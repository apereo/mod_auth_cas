...
static int cas_char_to_env(int c) {
...
static int cas_strnenvcmp(const char *a, const char *b, int len) {
	int i = 0;
	while (1) {
 ...
 * recorded/returned.
 */
static apr_table_t *cas_scrub_headers(
		apr_pool_t *p,
		...
		)
{
	...
	/* If the caller wants the dirty headers, then given them a
	 * pointer. */
...
static void cas_scrub_request_headers(
		request_rec *r,
